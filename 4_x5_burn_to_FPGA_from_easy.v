...
begin
	...
	begin // 取指令 + 分析指令 + 执行 | 或 准备数据 (分析且备好该指令所需的数据）
	   ...
	   //32'b0000000_?????_?????_101_?????_0111011: re[w_rd] <= (re[w_rs2][4:0] == 0) ? {{32{re[w_rs1][31]}}, re[w_rs1][31:0]} : {{32{re[w_rs1][31]}}, re[w_rs1][31:0] >> re[w_rs2][4:0]}; // Srlw
	   32'b0000000_?????_?????_101_?????_0111011: re[w_rd] <= {{32{(re[w_rs1][31:0] >> re[w_rs2][4:0])[31]}}, re[w_rs1][31:0] >> re[w_rs2][4:0]}; // Srlw
