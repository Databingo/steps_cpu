...
#define ABORT_NAK_DELAY		64		//放弃NAK重试命令延时
#define USB_IN_DELAY		256		//中断传输IN命令延时
...
static unsigned char		USB_Key_Sync[DEVICE_NUM];			//键盘同步触发控制
static unsigned char		USB_Mouse_Sync[DEVICE_NUM];			//鼠标同步触发控制
static unsigned char		USB_Current_Sync[DEVICE_NUM];		//当前操作的同步触发控制
	...
	for (device = 0; device < (unsigned char)DEVICE_NUM; ++device) {
		...
		if (CH375_Flag[device] == (unsigned char)1 && CH375_GET_INT(device) == (unsigned char)0) {
			...
			if (initcode == (unsigned char)0x14) {
				...
				USB_Current_Sync[device] = ~USB_Current_Sync[device];
			}
			//设备插入
		...
		if ((USB_Device_Code[device] & (DEVICE_KEY | DEVICE_MOUSE)) != (unsigned char)0) {
			...
			if (USB_Device_Code[device] == (DEVICE_KEY | DEVICE_MOUSE)) {
				...
				if (USB_Current_Endpoint[device] == USB_Key_Endpoint[device]) {
					USB_Current_Endpoint[device] = USB_Mouse_Endpoint[device];
					USB_Key_Sync[device] = USB_Current_Sync[device];
					USB_Current_Sync[device] = USB_Mouse_Sync[device];
				} else {
					USB_Current_Endpoint[device] = USB_Key_Endpoint[device];
					USB_Mouse_Sync[device] = USB_Current_Sync[device];
					USB_Current_Sync[device] = USB_Key_Sync[device];
				}
			}
			CH375_USB_IN(device, USB_Current_Sync[device], USB_Current_Endpoint[device]); //执行IN事务查询设备
			Delay(USB_IN_DELAY);
		}
	}
	return update;
