...
//颜色数组
//unsigned char k1[] = { 0xe0,0xf8,0xfc,0x1c,0x03,0x0f,0xeb,0x00,0xff,0x06 };	 //红、橙、黄、绿、蓝、靛、紫、黑、白、天蓝  高八位
unsigned char k1[] = { 0xe0,0xf8,0xfc,0x1c,0x03,0x0f,0xeb,0x00,0xff,0x0e };	 //红、橙、黄、绿、蓝、靛、紫、黑、白、天蓝  高八位
//unsigned char k2[] = { 0x00,0x00,0xe0,0xe0,0x1f,0xff,0x1f,0x00,0xff,0x17 };	 //红、橙、黄、绿、蓝、靛、紫、黑、白、天蓝  低八位
...
unsigned char k3[] = { 0x1f,0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x1f,  //0
				   ...
				   0x11,0x11,0x11,0x11,0x1f,0x01,0x01,0x01,0x01,  //4	 //数字显示0~9字库 5*9像素
				   0x1f,0x10,0x10,0x10,0x1f,0x01,0x01,0x01,0x1f,  //5	 //每字节的低5位分别对应每行的5个像素
				   0x1f,0x10,0x10,0x10,0x1f,0x11,0x11,0x11,0x1f,  //6	 //一个字模共有9行就即9个字节
...
unsigned char p1, p2, p3;  //p1是写液晶命令低字节 p2是写液晶数据高字节  p3是写液晶数据低字节

unsigned char se = 2;  //玩家颜色变量 2即对应以上颜色数组的黄色

unsigned char se1; //豆子的颜色变量

unsigned char sex; //那颗闪硕的豆子的颜色计数变量

unsigned char se2 = 9; //玩家和鬼还有生命显示共用的“擦除帧”的颜色变量(为背景色)

unsigned char yy, xx; //玩家和鬼还有生命显示共用的“擦除帧”的坐标变量 分别对应Y、X

unsigned char yy1, xx1; //玩家像素帧真实坐标变量 分别对应Y、X

unsigned char yy2, xx2; //鬼像素帧的共用坐标 分别对应Y、X

unsigned char yy3, xx3; //豆子像素帧的真实坐标 分别对应Y、X

unsigned char k; //玩家面对的方向控制 共有4个值	等于0时对右张嘴闭嘴 等于1时对左张嘴闭嘴 等于2时对上张嘴闭嘴	等于3时对下张嘴闭嘴

unsigned char kkk; //玩家张嘴闭嘴和鬼伸脚缩脚控制 为0时玩家张嘴鬼伸脚 为1时玩家闭嘴鬼缩脚

unsigned char j; //玩家张嘴闭嘴和鬼伸脚缩脚频率控制计数变量 

unsigned char ky, kx; //画地图用的11*11像素模的坐标变量 分别对应Y、X   X轴起始值为4(边距)

unsigned char ghost_num; //鬼的数量
unsigned char ghost_x[20]; //鬼的真实坐标x
unsigned char ghost_y[20]; //鬼的真实坐标y
unsigned char ghost_display[20]; //鬼的显示开关
unsigned int ghost_delayed[20]; //鬼的出击延时计数
unsigned char ghost_xy_add_sub[20]; //鬼的真实X、Y坐标增减量控制台开关

//unsigned char xxx1, yyy1, xxx2, yyy2, xxx3, yyy3, xxx4, yyy4, xxx5, yyy5, xxx6, yyy6, xxx7, yyy7, xxx8, yyy8, xxx8, yyy8;//4个鬼的真实坐标 分别对应X、Y

//unsigned char c1, c2, c3, c4, c5, c6, c7, c8; //4个鬼的总开关 为0时关闭 鬼不显示 为1时启动鬼的显示

//unsigned char s1, s2, s3, s4, s5, s6, s7, s8; //4个鬼的出击延时计数  达到条件时分别对应开启c1,c2,c3,c4

//unsigned char jj2, jj3, jj4, jj5; //4个鬼的真实X、Y坐标增减量控制台开关

unsigned char jj1; //玩家X、Y坐标增减量控制台开关

unsigned char jjj; //鬼共用X、Y坐标增减量控制台开关

unsigned char xxxx, yyyy; //鬼控制台共用坐标变量 4个鬼的镜像坐标

unsigned char sm; //生命数变量

unsigned char smx; //生命显示用的循环计数变量

unsigned char yy1jc, xx1jc; //生命显示用的玩家坐标寄存变量

unsigned char z1, z2, z3, z4, z5, z6, z7, z8, z9, z10, z11, z12, z13, z14, z15, z16, z17, z18, z19, z20;  //豆子刷新开关(20颗豆子) 为0时刷新豆子 为1时关闭豆子刷新

unsigned char wd; //玩家无敌总开关变量 为0时打开无敌判断 为1时关闭无敌判断

unsigned char xxxxx, yyyyy; //玩家和鬼碰撞判断共用坐标 4个鬼的镜像坐标

unsigned char cc; //玩家和鬼碰撞判断共用变量 这个变量是c1,c2,c3,c4的镜像变量 

unsigned char zm; //数字显示字模位置(90进制)0,9,18,27,36,45,54,63,72,81 分别对应0~9

unsigned char sx, sy; //数字字模显示坐标 分别对应X、Y

unsigned char zzzz1, zzzz2, zzzz3, zzzz4; //数字显示进位寄存变量 依次最低位到最高位(90进制)

unsigned char xh = 0; //第2死循环真假值

unsigned int y; //刷绿色障碍块计数和玩家无敌延时时间共用变量

unsigned int x; //共用延时计数和计数变量

unsigned char z; //游戏速度变量

unsigned char chidouzi_KEY_D_bz = 0; //按键步骤控制

unsigned char KEY_O = 0; //方向键上  
unsigned char KEY_U = 0; //方向键下
unsigned char KEY_L = 0; //方向键左
unsigned char KEY_R = 0; //方向键右
unsigned char KEY_S = 0; //选择键(此游戏用不到)
unsigned char KEY_D = 0; //确定键	按下开始游戏
unsigned char KEY_A = 0; //攻击键(此游戏用不到)
unsigned char KEY_B = 0; //跳跃键(此游戏用不到)


void chidouzi_Display_interface_0(unsigned char y, unsigned char x, unsigned char data) //显示接口0,画点,yx为坐标,data像素数据
{
	unsigned short xx = x, yy = y;
	//Lcd_Draw_Pixel(x, y, data);

	Lcd_WriteIndex(0x2a);
	Lcd_WriteData_16Bit_V(xx);
	Lcd_WriteIndex(0x2b);
	Lcd_WriteData_16Bit_V(yy);
	Lcd_WriteIndex(0x2c);
	Lcd_WriteData_8Bit332(data);
}

void chidouzi_Display_interface_1(unsigned char y, unsigned char x, unsigned char high, unsigned char wieth, unsigned char data) //显示接口1,画色块,yx为起始坐标,_y_x为点数,data像素数据
{
	//Lcd_Fill_Rectangle(x, y, _x, _y, data);

	unsigned short all, i;

	if (wieth == (unsigned char)0 || high == (unsigned char)0)
		return;
	Lcd_SetRegion(x, x + (unsigned short)wieth - (unsigned short)1, y, y + (unsigned short)high - (unsigned short)1);
	all = (unsigned short)wieth * high;
	Lcd_WriteIndex(0x2c);
	for (i = 0; i < all;) {
		if (all - i >= 8) {
			Lcd_WriteData_8Bit332(data);
