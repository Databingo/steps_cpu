...
//
//int main() {
//    manual_puts("1. Software Workaround Start\n");
//
//    mkdir("/dev", 0755);
//    mount("devtmpfs", "/dev", "devtmpfs", 0, NULL);
//
//    // WORKAROUND: Map a "Safe" High-Memory Page (At 4GB mark)
//    // This address has bit 32 set, so (va[63:16] == 0) will be FALSE.
//    void *high_mem = mmap((void*)0x100000000, 4096, PROT_READ | PROT_WRITE, 
//                          MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0);
//
//    if (high_mem == MAP_FAILED) {
//        manual_puts("Mmap failed! Trying lower but safe address...\n");
//        high_mem = mmap((void*)0x80000000, 4096, PROT_READ | PROT_WRITE, 
//                        MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0);
//    }
//
//    // Define "Safe" pointers in high memory
//    char *safe_console = (char *)high_mem;
//    char *safe_kmsg = (char *)high_mem + 100;
//    struct stat *safe_stat = (struct stat *)((char *)high_mem + 512);
//
//    // Copy paths to high memory
//    strcpy(safe_console, "/dev/console");
//    strcpy(safe_kmsg, "/dev/kmsg");
//
//    // Force create nodes just in case devtmpfs is slow
...
//    manual_puts("open(/dev/console) result: ");
//    manual_print_int(fd);
//
//    if (fd >= 0) {
//        // Force the console onto FD 0, 1, 2
//        dup2(fd, 0);
//        dup2(fd, 1);
//        dup2(fd, 2);
//        if (fd > 2) close(fd);
//
//        manual_puts("2. FDs 0,1,2 mapped to console.\n");
//
//        // Test fcntl on FD 1
//        int flags = fcntl(1, F_GETFL);
//        manual_puts("fcntl(1) result: ");
//        manual_print_int(flags);
//
//        // Test fstat on FD 1 using high memory buffer
//        int st_res = fstat(1, safe_stat);
//        manual_puts("fstat(1) result: ");
//        manual_print_int(st_res);
//    }
//
//    manual_puts("3. Testing final printf...\n");
//    printf("PRINTF WORKAROUND SUCCESS!\n");
//    fflush(stdout);
//
//    while(1) { sleep(1); }
//    return 0;
