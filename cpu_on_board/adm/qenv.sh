...
# Also use it if we are running jtagd, since it interacts with hardware it must always match the OS
...
if test "${USE_64BIT-UNSET}" != UNSET ; then
	PLATFORM_DIR=${QUARTUS_PLATFORM}64
else
	PLATFORM_DIR=$QUARTUS_PLATFORM
fi

# The QUARTUS_BINDIR is standard in our environment, like QUARTUS_ROOTDIR
export QUARTUS_BINDIR=$QUARTUS_ROOTDIR/$PLATFORM_DIR
...
if test ! -d $QUARTUS_BINDIR ; then
	echo "*** The Quartus II software cannot be started because the current platform,"
	echo "    '$PLATFORM_DIR', does not appear to be installed in:"
