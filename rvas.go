...
func main() { //t6a7s11
	...
	opBin := map[string]uint32{
		...
		"amomin.w":  0b10000000000000000010000000101111, // func5=10100, func3=010
		"amomax.w":  0b10100000000000000010000000101111, // func5=10000, func3=010
		"amominu.w": 0b11000000000000000010000000101111, // func5=11100, func3=010
		"amomaxu.w": 0b11100000000000000010000000101111, // func5=11000, func3=010
		...
		"amomin.d":  0b10000000000000000011000000101111, // func5=10100, func3=011
		"amomax.d":  0b10100000000000000011000000101111, // func5=10000, func3=011
	...
	for scanner0.Scan() {
		...
		if strings.HasPrefix(switchOnOp, ".") {
			...
			if directive == ".dword" {
			    ...
			    integer, _ := strconv.ParseUint(suf_directive, 0, 64)
			    buf := make([]byte, 8)
			    binary.LittleEndian.PutUint64(buf, uint64(integer))
			    ...
			    buf =  align_x(buf, 8)
	                    ...
	                    symtab_[sym_index].Shndx = uint16(slices.Index(shstrtab, section_in))//4 //uint16 // section index the symbol in (.text)
	                    symtab_[sym_index].Value = uint64(len(data)) //# uint64  for relocatable .o file it's symbol's offset in its section
	                    symtab_[sym_index].Size = 8  //

			    //sym + str + data
                            //data = append(data, pad8...)
                            data = append(data, buf...)
	                    } else {
				fmt.Println("Error: Label not found for .dword", integer)
				os.Exit(1) }
			}
