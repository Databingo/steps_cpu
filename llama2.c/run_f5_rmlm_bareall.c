...
// Custom math function implementations
float sqrtf(float x) {
    if (x < 0.0f) return 0.0f; // Handle negative numbers
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (guess == prev_guess) break; // Converged
    }
...
float expf(float x) {
    ...
    if (x > 88.0f) return 1e38f; // Overflow approximation
    ...
    // Calculate 2^f using polynomial approximation
    // 2^f ≈ 1 + f + 0.5*f^2 + 0.166667*f^3 + 0.041667*f^4
    float result = 1.0f + f * (1.0f + f * (0.5f + f * (0.166667f + f * 0.041667f)));
    
    // Apply integer part by bit manipulation
    if (n >= 0) {
        // For positive n, multiply by 2^n
        unsigned int bits = *(unsigned int*)&result;
        bits += n << 23; // Add n to exponent
        result = *(float*)&bits;
    } else {
        // For negative n, divide by 2^|n|
        unsigned int bits = *(unsigned int*)&result;
        bits -= (-n) << 23; // Subtract |n| from exponent
        result = *(float*)&bits;
    }
...
#define ARENA_SIZE 128000000 // Increased to 128MB for larger models
...
void build_transformer(Transformer* t)
{
    ...
    // Print model info
    char buf[32];
    uart_puts("Model config: dim="); 
    // Simple itoa for model info
    int n = t->config.dim;
    int i = 0;
    char temp[16];
    if (n == 0) { uart_puts("0"); } else {
        while (n > 0) { temp[i++] = '0' + (n % 10); n /= 10; }
        while (i > 0) uart_putc(temp[--i]);
    }
    uart_puts(" layers=");
    n = t->config.n_layers;
    i = 0;
