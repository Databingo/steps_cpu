...
def version2_export(model, filepath, group_size=64):
    ...
    shared_classifier = torch.equal(model.tok_embeddings.weight, model.output.weight)
    if not shared_classifier:
        weight_categories.append(model.output.weight)
    
    # Validate all weights
    for i, w in enumerate(weight_categories):
    ...
    # now let's write out all the params that we are quantizing to Q8_0 incrementally
    maxerr = 0
    for i, w in enumerate(weight_categories):
        ...
        del q, s
        # logging
        print(f"{i+1}/{len(weight_categories)} quantized {tuple(w.shape)} to Q8_0 with max error {err}")

    print(f"max quantization group error across all weights: {maxerr}")
...
def load_hf_model(model_path, dtype=torch.float16):
    """Load HuggingFace model with reduced memory usage"""
    try:
    ...
    # load HF model with low memory settings
    ...
    # Free HuggingFace model memory immediately
    del hf_model, hf_dict
    gc.collect()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dtype", type=str, help="dtype of the model (fp16, fp32)", default="fp16")  # Changed default to fp16
    ...
    if args.checkpoint:
        ...
        model = load_hf_model(args.hf, dtype=dtype)  # Pass dtype to loader
