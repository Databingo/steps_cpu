...
void read_line(char* buffer, int max_len) {
    ...
    while (i < max_len - 1) {
        char c = uart_getc();
        if (c == '\r') { uart_puts("\r\n"); break; }
        else if (c == 127 || c == '\b') { if (i > 0) { i--; uart_puts("\b \b"); } }
        else if (c >= 32 && c < 127) { buffer[i++] = c; uart_putc(c); }
    }
    buffer[i] = '\0';
}

// libm function declarations
float sqrtf(float); float expf(float);
...
void build_transformer(Transformer *t) {
    ...
    w->token_embedding_table = weights_ptr; weights_ptr += p->vocab_size * p->dim;
    w->rms_att_weight = weights_ptr; weights_ptr += p->n_layers * p->dim;
    w->wq = weights_ptr; weights_ptr += p->n_layers * p->dim * (p->n_heads * head_size);
    w->wk = weights_ptr; weights_ptr += p->n_layers * p->dim * (p->n_kv_heads * head_size);
    w->wv = weights_ptr; weights_ptr += p->n_layers * p->dim * (p->n_kv_heads * head_size);
    w->wo = weights_ptr; weights_ptr += p->n_layers * (p->n_heads * head_size) * p->dim;
    w->rms_ffn_weight = weights_ptr; weights_ptr += p->n_layers * p->dim;
    w->w1 = weights_ptr; weights_ptr += p->n_layers * p->dim * p->hidden_dim;
    w->w2 = weights_ptr; weights_ptr += p->n_layers * p->hidden_dim * p->dim;
    w->w3 = weights_ptr; weights_ptr += p->n_layers * p->dim * p->hidden_dim;
    w->rms_final_weight = weights_ptr; weights_ptr += p->dim;
    ...
    s->x = arena_alloc(p->dim * sizeof(float)); s->xb = arena_alloc(p->dim * sizeof(float));
    s->xb2 = arena_alloc(p->dim * sizeof(float)); s->hb = arena_alloc(p->hidden_dim * sizeof(float));
    s->hb2 = arena_alloc(p->hidden_dim * sizeof(float)); s->q = arena_alloc(p->dim * sizeof(float));
    s->k = arena_alloc(kv_dim * sizeof(float)); s->v = arena_alloc(kv_dim * sizeof(float));
