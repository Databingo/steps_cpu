 ...
 * Based on the working run_f.c with added dequantization support
...
// ----------------------------------------------------------------------------
// Globals and Data Structures (Based on run_f.c)
typedef struct {
    int dim;
    int hidden_dim;
    int n_layers;
...
void build_transformer(Transformer* t) {
    ...
    // Read header field-by-field to prevent padding issues
    ...
    uint8_t shared_classifier = *(uint8_t*)(model_ptr + offset);
    offset += 1;
    GS = *(int*)(model_ptr + offset);
    ...
    // Map float weights (same as run_f.c)
    ...
    // Map quantized weights and dequantize them
    ...
    for (int l = 0; l < p->n_layers; l++) {
        dequantize(q_wq + l, w->wq + l * p->dim * (p->n_heads * head_size), p->dim * (p->n_heads * head_size));
    }

    QuantizedTensor* q_wk = init_qtensor(&weights_ptr, p->n_layers, p->dim * (p->n_kv_heads * head_size));
    w->wk = arena_alloc(p->n_layers * p->dim * (p->n_kv_heads * head_size) * sizeof(float));
    for (int l = 0; l < p->n_layers; l++) {
        dequantize(q_wk + l, w->wk + l * p->dim * (p->n_kv_heads * head_size), p->dim * (p->n_kv_heads * head_size));
    }

    QuantizedTensor* q_wv = init_qtensor(&weights_ptr, p->n_layers, p->dim * (p->n_kv_heads * head_size));
    w->wv = arena_alloc(p->n_layers * p->dim * (p->n_kv_heads * head_size) * sizeof(float));
    for (int l = 0; l < p->n_layers; l++) {
        dequantize(q_wv + l, w->wv + l * p->dim * (p->n_kv_heads * head_size), p->dim * (p->n_kv_heads * head_size));
    }

    QuantizedTensor* q_wo = init_qtensor(&weights_ptr, p->n_layers, (p->n_heads * head_size) * p->dim);
    w->wo = arena_alloc(p->n_layers * (p->n_heads * head_size) * p->dim * sizeof(float));
    for (int l = 0; l < p->n_layers; l++) {
        dequantize(q_wo + l, w->wo + l * (p->n_heads * head_size) * p->dim, (p->n_heads * head_size) * p->dim);
    }

    QuantizedTensor* q_w1 = init_qtensor(&weights_ptr, p->n_layers, p->dim * p->hidden_dim);
    w->w1 = arena_alloc(p->n_layers * p->dim * p->hidden_dim * sizeof(float));
    for (int l = 0; l < p->n_layers; l++) {
        dequantize(q_w1 + l, w->w1 + l * p->dim * p->hidden_dim, p->dim * p->hidden_dim);
    }

    QuantizedTensor* q_w2 = init_qtensor(&weights_ptr, p->n_layers, p->hidden_dim * p->dim);
    w->w2 = arena_alloc(p->n_layers * p->hidden_dim * p->dim * sizeof(float));
    for (int l = 0; l < p->n_layers; l++) {
        dequantize(q_w2 + l, w->w2 + l * p->hidden_dim * p->dim, p->hidden_dim * p->dim);
    }

    QuantizedTensor* q_w3 = init_qtensor(&weights_ptr, p->n_layers, p->dim * p->hidden_dim);
    w->w3 = arena_alloc(p->n_layers * p->dim * p->hidden_dim * sizeof(float));
    for (int l = 0; l < p->n_layers; l++) {
        dequantize(q_w3 + l, w->w3 + l * p->dim * p->hidden_dim, p->dim * p->hidden_dim);
    }
    ...
    if (shared_classifier) {
        ...
        QuantizedTensor* q_wcls = init_qtensor(&weights_ptr, 1, p->dim * p->vocab_size);
        w->wcls = arena_alloc(p->dim * p->vocab_size * sizeof(float));
        dequantize(q_wcls, w->wcls, p->dim * p->vocab_size);
    }

    // Allocate RunState (same as run_f.c)
...
// Copy all the working functions from run_f.c
...
// Copy all tokenizer and sampler functions from run_f.c
