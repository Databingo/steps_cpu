 ...
 * Fixed dequantize function to process all elements and prevent repetitive
 * token output. Added debug checks to identify hanging issues during
 * initialization.
 */
...
void encode(Tokenizer *t, char *text, int8_t bos, int8_t eos, int *tokens,
            int *n_tokens);
void simple_qsort(void *base, size_t nitems, size_t size,
...
void dequantize(QuantizedTensor *qx, float *x, int n) {
  ...
  uart_puts("     - qx->q pointer: ");
  itoa((int)(size_t)qx->q, buf);
  uart_puts(buf);
  uart_puts("\n");
  uart_puts("     - qx->s pointer: ");
  itoa((int)(size_t)qx->s, buf);
  uart_puts(buf);
  uart_puts("\n");
  uart_puts("     - x pointer: ");
  itoa((int)(size_t)x, buf);
  uart_puts(buf);
  uart_puts("\n");
  uart_puts("     - n value: ");
  itoa(n, buf);
  ...
  for (int group = 0; group < num_groups; group++) {
    ...
    if (scale == 0.0f) {
      uart_puts("WARNING: Zero scale in group ");
      itoa(group, buf);
      uart_puts(buf);
      uart_puts("\n");
...
QuantizedTensor *init_qtensor(unsigned char **ptr, int n, int size_each) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (misalignment != 0) {
      *ptr += (4 - misalignment);
      uart_puts("     - Aligned ptr for qtensor ");
      itoa(i, buf);
      uart_puts(buf);
      uart_puts(" by ");
      itoa(4 - misalignment, buf);
      uart_puts(buf);
      uart_puts(" bytes\n");
    }
    res[i].s = (float *)*ptr;
...
void build_transformer(Transformer *t) {
  ...
  uart_puts("     - Config: dim=");
  itoa(t->config.dim, buf);
  uart_puts(buf);
  uart_puts(" hidden_dim=");
  itoa(t->config.hidden_dim, buf);
  uart_puts(buf);
  uart_puts(" n_layers=");
  itoa(t->config.n_layers, buf);
  uart_puts(buf);
  uart_puts(" vocab_size=");
  itoa(t->config.vocab_size, buf);
  uart_puts(buf);
  uart_puts(" seq_len=");
  itoa(t->config.seq_len, buf);
  uart_puts(buf);
  uart_puts("\n     - GS=");
  itoa(GS, buf);
  uart_puts(buf);
  ...
  w->token_embedding_table =
      arena_alloc(p->vocab_size * p->dim * sizeof(float));

  uart_puts("   - Dequantizing token embeddings...\n");
  dequantize(w->q_tokens, w->token_embedding_table, p->vocab_size * p->dim);

  uart_puts("   - Mapping attention weights...\n");
  w->wq = init_qtensor(&weights_ptr, p->n_layers,
                       p->dim * (p->n_heads * head_size));
  w->wk = init_qtensor(&weights_ptr, p->n_layers,
                       p->dim * (p->n_kv_heads * head_size));
  w->wv = init_qtensor(&weights_ptr, p->n_layers,
                       p->dim * (p->n_kv_heads * head_size));
  w->wo = init_qtensor(&weights_ptr, p->n_layers,
  ...
  w->wcls = shared_classifier
                ? w->q_tokens
                : init_qtensor(&weights_ptr, 1, p->dim * p->vocab_size);
  ...
  s->value_cache =
      arena_alloc(p->n_layers * p->seq_len * kv_dim * sizeof(float));
...
float *forward(Transformer *t, int token, int pos) {
  ...
  if (token < 0 || token >= p->vocab_size) {
    char buf[32];
    uart_puts("ERROR: Invalid token ID in forward: ");
    itoa(token, buf);
    uart_puts(buf);
    uart_puts("\n");
    return s->logits;
  ...
  for (int l = 0; l < p->n_layers; l++) {
    ...
    for (int h = 0; h < p->n_heads; h++) {
      ...
      for (int t_step = 0; t_step <= pos; t_step++) {
        float *k_t =
            s->key_cache + loff + t_step * kv_dim + (h / kv_mul) * head_size;
        float score = 0.0f;
      ...
      for (int t_step = 0; t_step <= pos; t_step++) {
        float *v_t =
            s->value_cache + loff + t_step * kv_dim + (h / kv_mul) * head_size;
        float a = att[t_step];
        for (int i = 0; i < head_size; i++) {
          xb[i] += a * v_t[i];
        }
...
void simple_qsort(void *b, size_t n, size_t s,
...
void build_tokenizer(Tokenizer *t, int vocab_size) {
  ...
  uart_puts("     - Max token length: ");
  itoa(t->max_token_length, buf);
  uart_puts(buf);
  uart_puts("\n");
  offset += sizeof(int);
  for (int i = 0; i < vocab_size; i++) {
    ...
    offset += sizeof(int);
    if (len <= 0 || len > 256) {
      uart_puts("ERROR: Invalid token length at index ");
      itoa(i, buf);
      uart_puts(buf);
      uart_puts(": ");
      itoa(len, buf);
...
char *decode(Tokenizer *t, int prev, int token) {
  ...
  if (token < 0 || token >= t->vocab_size) {
    uart_puts("ERROR: Invalid token ID: ");
    itoa(token, buf);
    uart_puts(buf);
    uart_puts("\n");
    return "";
  ...
  uart_puts("Decode token ID: ");
  itoa(token, buf);
  uart_puts(buf);
  uart_puts(" -> ");
  uart_puts(t->vocab[token]);
  uart_puts("\n");
...
void build_sampler(Sampler *s, int vocab_size, float temp,
                   unsigned long long seed) {
  s->vocab_size = vocab_size;
  s->temperature = temp;
  s->rng_state = seed;
}
void generate(Transformer *t, Tokenizer *tok, Sampler *sampler, char *prompt,
              int steps) {
  char buf[64];
  int num_prompt;
...
int main() {
  ...
  uart_puts("Bare-metal INT8 Llama2.c for "
            "RISC-V\n--------------------------------\n");
