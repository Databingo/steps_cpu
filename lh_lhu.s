...
## TEST: LH_POS
    # Purpose: Load positive halfword (0x1234 at Addr 4), check sign extension.
...
## TEST: LH_NEG
    # Purpose: Load negative halfword (0x8000 at Addr 6 = -32768), check sign extension.
    # Data @004: 34 12 00 80
...
## TEST: LH_NEG_ONE
    # Purpose: Load negative one halfword (0xFFFF at Addr 0x30 = 48), check sign extension.
    # Data @048: FF FF FF 7F
    lh  x31, 48(x5)           # Load half: x31 = MEM[0 + 48] sign-extended
    li  x30, 0xFFFFFFFFFFFFFFFF # Golden value (-1)
    li  x11, 1                 # Signal Compare
    li  x11, 0                 # Clear Signal

## TEST: LH_MAX_POS
    # Purpose: Load max positive halfword (0x7FFF at Addr 0x32 = 50), check sign extension.
    # Data @048: FF FF FF 7F
    lh  x31, 50(x5)           # Load half: x31 = MEM[0 + 50] sign-extended
...
## TEST: LHU_POS
    # Purpose: Load positive halfword (0x1234 at Addr 4), check zero extension.
    # Data @004: 34 12 00 80
...
## TEST: LHU_NEG_PATTERN
    # Purpose: Load halfword with MSB set (0x8000 at Addr 6), check zero extension.
    # Data @004: 34 12 00 80
...
## TEST: LHU_NEG_ONE_PATTERN
    # Purpose: Load halfword pattern 0xFFFF at Addr 0x30 (48), check zero extension.
    # Data @048: FF FF FF 7F
    lhu x31, 48(x5)           # Load half: x31 = MEM[0 + 48] zero-extended
    li  x30, 0x000000000000FFFF # Golden value
    li  x11, 1                 # Signal Compare
    li  x11, 0                 # Clear Signal

## TEST: LHU_MAX_POS
    # Purpose: Load max positive halfword (0x7FFF at Addr 0x32 = 50), check zero extension.
    # Data @048: FF FF FF 7F
    lhu x31, 50(x5)           # Load half: x31 = MEM[0 + 50] zero-extended
    li  x30, 0x0000000000007FFF # Golden value
    li  x11, 1                 # Signal Compare
    li  x11, 0                 # Clear Signal
...
## TEST: LH_NEGATIVE_OFFSET
    # Purpose: Load half 0x1234 (Addr 4) using base x5=6 and offset -2. Check sign extension.
