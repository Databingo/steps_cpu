...
func main() {
	...
	for _, seti := range sections {
		for _, mc_line := range seti.lines {
			_, err := assemble(params, mc_line, instrExpr, loadRefExpr, labels)
			if err != nil {
				fmt.Println(err)
				os.Exit(-1)
			}
...
func assemble(parms map[string]interface{}, mc_line *machine_line, expr *regexp.Regexp, loadRefExpr *regexp.Regexp, labels map[string]string) (macCode string, err error) {
